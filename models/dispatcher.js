...
dispatcher.prototype.checkMuteButton = function checkMuteButton()
{
...
dispatcher.prototype.createExtraSlot = function createExtraSlot() 
{
...
dispatcher.prototype.ConrolePaused = function ConrolePaused(isClickable) 
{
...
dispatcher.prototype.parseConfig = function parseConfig() 
{
    var vars = {};
...
dispatcher.prototype.start = function start() 
{
	var self= this;
	...
	var config=JSON.parse(txt); 
	   ...
	   self.tmpReadArCnt=self.links.length;
	   //console.log(JSON.stringify(config.ads[1]));
...
dispatcher.prototype.checkLoadedQueue = function checkLoadedQueue() {
 var x;
 var i=0;
 for(x in  this.cacheloadedIndexes){
  console.log([3187744,x,this.cacheloadedIndexes[x],i,this.tmpReadArCnt]);
...
dispatcher.prototype.pushQueue = function pushQueue(ind) {
  var self=this; 
   ...
   if(this.checkLoadedQueue()) {/*все загрузки вернулись*/
  ...
  var uri = this.links[ind].src.replace(/\{([a-z]+)\}/g, function(match)
    ...
    this.player.loadNew(uri).then(function getLoad(res){
	 ...
	 res.index=ind;
	 //if(self.links[ind].id==25){
	// console.log(['трисекунды',res]);
	 //} 
...
dispatcher.prototype.changeIndex = function changeIndex(index) {
...
dispatcher.prototype.startPlayQueue = function startPlayQueue() {
this.emptyStart=1;
var self=this;
 ...
 this.reporter=self.resourcesToPlay[index];
  this.player.once('AdError', function(reason) {
    ...
    mess=JSON.stringify(reason);
			  ...
			  console.log([3185,index,'Плеер ошибся в загрузку!',self.links[index].title,mess]); 
		     ...
		     this.container.innerHTML='';
			  ...
			  self.sendStatistic({id:self.links[index].id,eventName:'errorPlayMedia',mess:mess}); 
			  console.log([3185,index,'Плеер ошибся!',self.links[index].title]); 
			  //self.changeIndex(index);
              });
			  self.player.once('AdStopped', function() {
			
			  console.log([31869,'Плеер отыграл!',self.links[index].title]);
			  self.changeIndex(index);
			  });
			  self.player.TriodAction=function(args){
              if(typeof args.index=='undefined' || args.index<0) return; 
			   ...
			   delete self.player.TriodAction;
			   self.player.TriodAction=null
             }
			 //if(self.player.__private__.pType!=1)  
			  ...
			  console.log([31869,'Плеер заиграл!',self.player.adVolume,self.links[index].title]);
			  self.sendStatistic({id:self.links[index].id,eventName:'startPlayMedia'});
			  }).catch(function onError(e){
			    console.log([318666,'Плеер сдох!',e]);
			    self.changeIndex(index);
			  });
			  ...
			  //self.startPlayQueue();
			   ...
			   self.sendStatistic({id:self.links[ind].id,eventName:'srcLoadError',mess:mess});  
			   console.log([31865,'Error загрузки !',index,self.links[index].title,mess]);
			   self.changeIndex(index);
		   });
 
};
dispatcher.prototype.readyToPlayThird = function readyToPlayThird() {
...
console.log([1,"готов играть твигл"]); 
...
dispatcher.prototype.checkDefaultTrailer = function checkDefaultTrailer(cnt) {
	/*
	 var min=0;
	 var max=1;
	 var ran = Math.floor(Math.random() * (max -  min + 1)) + min;
	 this.player.createTrailer(this.defaultURLs[ran]);
	 return;
	 */
...
dispatcher.prototype.createTrailer=function(src) {
     this.container.innerHTML='';  
